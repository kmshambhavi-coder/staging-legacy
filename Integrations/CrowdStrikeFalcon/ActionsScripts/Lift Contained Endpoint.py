...
def main(is_first_run):
    ...
    try:
        ...
        for entity_type, entity_identifier in result_value["pending"].copy():
            ...
            try:
                ...
                # skip endpoints with status LIFT_CONTAINMENT_PENDING and process only the rest statuses
                    ...
                    DeviceStatusEnum.LIFT_CONTAINMENT_PENDING.value
                ):
                    siemplify.LOGGER.info(
        ...
        timeout_message = (
            "The following endpoints initiated containment lift, but were not able to finish it "
        ...
        if result_value["pending"]:
            if is_timeout_approached:
                ...
                output_message = (
                    f"Waiting for containment lift to finish for the following endpoints: "
        ...
        if status != EXECUTION_STATE_INPROGRESS and isinstance(result, dict):
            if result_value["ready"]:
                output_message += (
                    f"Successfully lifted containment on the following endpoints in {PRODUCT_NAME}: "
            ...
            if result["skipped"]:
                output_message += (
                    f"The following endpoints were not contained in {PRODUCT_NAME}: "
